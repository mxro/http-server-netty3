...
public class ShutdownServerFactory {
	...
	public static Netty3Server startNettyShutdownServer(final int port,
			final String secret, final ServerComponent shutdownOperations) {
		final ThreadFactory threadFactory = new ThreadFactory() {

			@Override
			public Thread newThread(final Runnable arg0) {

				return new Thread(arg0, "shutdown-server-"
		...
		final ServerBootstrap bootstrap = new ServerBootstrap(
				new NioServerSocketChannelFactory(newFixedThreadPool,
						newFixedThreadPool));

		final ShutdownRequestHandler shutdownRequestHandler = new ShutdownRequestHandler(
				secret, shutdownOperations);

		bootstrap.setPipelineFactory(new ShutdownServerPipelineFactory(
				shutdownRequestHandler));

		if (ENABLE_DEBUG) {
			System.out.println(ShutdownServerFactory.class
					+ ": Binding shutdown server on port: " + port);
		}

		if (!MxJREUtils.portAvailable(port)) {
			throw new IllegalStateException(
					"Cannot start shutdown server on port: " + port + "\n"
		...
		if (ENABLE_DEBUG) {
			System.out.println(ShutdownServerFactory.class
					+ ": Shutdown server started on port: " + port);
		}
		...
		return new InternalShutdownServer(port, server, shutdownRequestHandler,
				bootstrap);
