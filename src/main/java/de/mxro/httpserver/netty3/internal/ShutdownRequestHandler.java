...
import nx.server.netty.NxServerNetty;
...
public class ShutdownRequestHandler extends SimpleChannelUpstreamHandler {
				...
				shutdownOperations.stop(new ShutdownCallback() {
					...
					public void onSuccess() {
						...
						try {
							...
							// .printStackTrace();
							NxServerNetty.sendHttpSuccess(e,
									"Shutdown successful.".getBytes("UTF-8"),
									"text/plain");
					...
					public void onFailure(final Throwable t) {
						...
						System.out
								.println("Failure while shutting down server: "
										+ t.getMessage());
						t.printStackTrace();

						NxServerNetty.sendHttpSuccess(e, t.getMessage()
								.getBytes(), "text/plain");
	...
	public ShutdownRequestHandler(final String secret,
			final ServerComponent shutdownOperations) {
		super();
